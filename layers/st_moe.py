...
from layers.distributed import (
    AllGather,
    split_by_rank,
    gather_sizes,
    pad_dim_to,
...
class Expert(nn.Module):
    def __init__(self, dim, d_state=8, d_conv=4, expand=2, prenorm=False):
        super().__init__()
...
class Experts(nn.Module):
    ...
    def forward(
        ...
        # outs_dict = {}  # Dictionary to hold output tensors with expert identifiers

        # for idx, (expert, expert_input) in enumerate(zip(experts, x)):
        #     out = expert(expert_input)
        #     outs.append(out)
    
        #     expert_id = f"expert_{idx}"  # Example ID based on index
        #     outs_dict[expert_id] = out
...
class TopNGating(Module):
    ...
    def __init__(
        ...
        # self.to_gates1 = nn.Linear(dim//2, num_gates, bias = False)
    ...
    def forward(
        ...
        *_, b, group_size, dim, dtype, top_n, num_gates, eps = *x.shape, x.dtype, self.top_n, self.num_gates, self.eps
        ...
        # gate_logits = self.gelu(gate_logits)
        # gate_logits = self.to_gates1(gate_logits)

        # gate_logits = self.mamba(gate_logits)
...
class MoE(Module):
    ...
    def __init__(self,
        ...
        # Check if 'experts' is a single instance or a list
        if isinstance(experts, Module):
            experts = [experts for _ in range(num_experts)]
        elif experts is None:
            experts = [Expert(dim=dim) for _ in range(num_experts)]
    ...
    def forward(
        ...
        # Save output dictionary of experts
        # torch.save(outs_dict, './dataset/outs_dict.pth')
        # torch.save(expert_outputs, './visualization/expert_outputs.pth')
...
class SparseMoEBlock(Module):
    ...
    def forward(
        ...
        return MixtureOfExpertsReturn(x, 0, total_aux_loss, balance_loss, router_z_loss)